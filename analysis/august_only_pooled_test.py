"""
Same as august_only_test.py, but WITHOUT slot isolation: all August draws
(2PM+5PM+9PM combined, all years 2022-2026) are pooled into one training
set and one holdout set. This mixes draw slots together rather than
...
def main():
    ...
    august = august.sort_values(["date", "draw_time"]).reset_index(drop=True)
    print(f"Total August draws (all years, all slots POOLED together): {len(august)}\n")

    train = august.iloc[:-HOLDOUT_N]
    holdout = august.iloc[-HOLDOUT_N:]
    print(f"Train n={len(train)}, holdout n={len(holdout)} (slots mixed in both)\n")
